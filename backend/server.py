    ...
    if os.path.exists(alarm_file):
        try:
            with open(alarm_file, 'r', encoding='utf-8') as f:
                ...
                if 'event_video_pre_seconds' in config:
                    alarm_config['event_video_pre_seconds'] = max(0.0, float(config['event_video_pre_seconds']))
        ...
        def record_video():
            try:
                ...
                if not writer.isOpened():
                    backend_logger.error(f"报警事件视频录制失败：VideoWriter 打开失败 {filename}")
                    return
                ...
                for cached in cached_frames:
                    if cached.shape[0] != frame_h or cached.shape[1] != frame_w:
                        cached = cv2.resize(cached, (frame_w, frame_h))
                    writer.write(cached)
                    written += 1

                # 2) 再补录事件后帧，保证总时长为 duration
                ...
                while time.time() - start_ts < post_seconds:
                    ...
                    if frame_src is None:
                        time.sleep(0.02)
                        continue
                    frame_to_write = frame_src.copy()
                    if frame_to_write.shape[0] != frame_h or frame_to_write.shape[1] != frame_w:
                        frame_to_write = cv2.resize(frame_to_write, (frame_w, frame_h))
                    writer.write(frame_to_write)
                    written += 1
                    time.sleep(frame_interval)
                writer.release()

                if written > 0 and os.path.exists(filepath) and os.path.getsize(filepath) > 1000:
                    backend_logger.info(
                        f"报警事件视频已保存: {filename} ({os.path.getsize(filepath)} bytes, frames={written}, pre={pre_seconds:.1f}s, total={duration}s)"
                    )
                else:
                    backend_logger.error(
                        f"报警事件视频文件异常: {filename} (frames={written}, size={os.path.getsize(filepath) if os.path.exists(filepath) else 0} bytes)"
                    )
...
def trigger_drowsy_alarm(state, ear_value, mar_value, zone_name="当前区域"):
    ...
    alarm_data = {
        "time": current_time,
        "track_id": drowsy_track_id,
        "class_id": None,
        "class_name_cn": title,
        "object_name": title,
        "zone_id": drowsy_zone_id,
        "zone_name": zone_name,
...
def set_alarm_config():
    ...
    try:
        ...
        if 'event_video_pre_seconds' in data:
            pre_seconds = float(data['event_video_pre_seconds'])
            if pre_seconds < 0 or pre_seconds > 30:
                return jsonify({"success": False, "message": "事前缓存时长必须在0-30秒之间"}), 400
            alarm_config['event_video_pre_seconds'] = pre_seconds
