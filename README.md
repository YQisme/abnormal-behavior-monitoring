...
- 🧩 多档案管理：支持按模式切换模型档案与视频档案，并维护独立激活状态
- 📝 按模式独立日志展示：日志按 `profile` 分流，不同模式不重叠
- 📦 报警事件中心：支持报警事件列表、媒体访问与数据导出
- ✅ 离岗回岗确认：离岗监测支持回岗确认闭环，便于值守管理
- 🔊 报警联动：支持页面弹窗与声光报警联动（可配置）
...
以下内容基于 `main` 分支近期提交（`b9202ea` 之后，含 `e8032f8`）整理，便于快速了解当前版本重点能力：

- **视频流与档案激活优化**：改进视频流处理流程，增强检测档案/视频档案切换的稳定性与一致性
- **自动推理配置增强**：补充多档案自动启动推理配置与状态管理，服务启动后可按档案自动运行
- **报警事件能力增强**：新增并持续完善报警事件列表、媒体访问、前端展示与导出能力
- **离岗监测闭环**：新增回岗确认功能，便于将离岗告警与回岗动作关联
- **报警联动增强**：新增弹窗与声光报警配置，提升现场告警可感知性
- **检测线程优化**：改进检测工作线程内存管理与检测逻辑，提升长时运行稳定性
- **瞌睡监测持续完善**：补充瞌睡监测相关检测与配置能力，支持三模式协同管理
- **本地音频报警能力**：新增后端本地语音播报链路（瞌睡/离岗），支持音频文件配置、前端试听与播放器回退策略
- **推理日志增强**：补充推理后端日志处理逻辑，便于按模式排查推理行为

如需查看详细提交记录，可在项目根目录执行：
...
# 2. 安装系统依赖（如果需要视频录制、报警事件保存、本地语音播报）
# Ubuntu/Debian
sudo apt-get update
sudo apt-get install -y ffmpeg mpg123

# CentOS/RHEL
sudo yum install -y ffmpeg mpg123
```
...
#### 问题 6：前端试听正常，但后端本地播报是噪音/无声

**常见原因**：
- 使用 `aplay` 直接播放 `.mp3` 会出现噪音（未解码）
- 系统未安装 `mpg123`，且 `ffplay` 不可用
- `audio/` 目录文件名与报警设置不一致
...
**说明**：当前后端策略为 `.mp3 -> mpg123/ffplay`，`.wav -> aplay`。
...
- **报警设置**：
  ...
  - 本地语音播报（后端）：
    - 开关：`后端本地播报`
    - 设备：`local_audio_device`（示例 `hw:0,0`）
    - 音量：`local_audio_volume`（0-100）
    - 最小间隔：`local_audio_min_interval`（防止连续重复播报）
    - 瞌睡监测音频文件（`audio/` 目录）：`drowsy.mp3` / `yawning.mp3` / `drowsy_yawning.mp3`
    - 离岗监测音频文件（`audio/` 目录）：`offpost.mp3`
    - 前端“试听”按钮通过 `/api/audio/preview?file=<name>` 播放对应音频
  - 点击"应用报警设置"使配置生效
...
- **本地语音播报文件目录**：
  - 将报警音频放在项目根目录 `audio/` 下（文件名与报警设置保持一致）
  - 建议至少准备：
    - `audio/drowsy.mp3`
    - `audio/yawning.mp3`
    - `audio/drowsy_yawning.mp3`
    - `audio/offpost.mp3`
  - 后端播放策略：
    - `.mp3`：优先 `mpg123`，回退 `ffplay`
    - 其他格式（如 `.wav`）：`aplay`
...
### v3.2.0
- ✨ 新增后端本地音频报警能力（瞌睡监测 + 离岗监测）
- ✨ 新增报警音频文件配置与前端试听能力（`audio/` 目录）
- ✨ 新增 `.mp3` 播放策略（优先 `mpg123`，回退 `ffplay`）
- ✨ 增强自动启动推理配置与多档案推理状态管理
- ✨ 增强报警事件处理链路与多档案联动能力
- ✨ 增强推理日志处理与前端可观测性
- 📝 README 按近期 git 历史补充完善

### v3.1.0
- ✨ 新增报警事件中心能力（事件列表、媒体访问、导出）
- ✨ 新增离岗回岗确认流程
- ✨ 新增弹窗与声光报警联动能力
- ✨ 优化视频流处理与多档案激活切换逻辑
- ✨ 完善瞌睡监测与检测配置管理能力
- ⚡ 优化检测线程内存管理，提升长时运行稳定性
- 📝 基于近期 git 历史补充 README 文档
